...
**Styling notes:** The component uses CSS variables (`--elevated`, `--border`, `--text-secondary`, `--text-muted`, `btn-primary`). For this site, map these to Grand China's equivalent variables or replace with hardcoded values matching the site's dark theme. Star color is Google yellow `#FBBC04`. Success state uses Google green `#34A853`.
